...
#define NTC_GPIO GPIO_NUM_32 // ADC1_CH4 VP
...
// Inicialização de hardware (Chame isso no app_main)
void init_hardware() {
    // Configura Buzzer
    ...
    // Configura ADC
    gpio_pullup_en(NTC_GPIO); // Ativa pull-up no pino VP (ADC1_CH0)
...
// Host Function: Lê o valor bruto do sensor
static int32_t host_read_adc(wasm_exec_env_t exec_env) {
    int raw_out = 0; // Inicialize com 0
    
    if (adc1_handle == NULL) {
        ESP_LOGE("HOST", "Erro: ADC handle ainda é NULO!");
        return 0;
    }

    esp_err_t ret = adc_oneshot_read(adc1_handle, ADC_CHANNEL, &raw_out);
    if (ret != ESP_OK) {
        return 0; // Se falhar a leitura, retorna 0 com segurança
    }
    
    return (int32_t)raw_out;
}

// Host Function: Controla o Buzzer
static void host_set_buzzer(wasm_exec_env_t exec_env, int32_t state) {
...
static NativeSymbol extended_native_symbols[] = {
    ...
    {
        .symbol = "read_adc",
        .func_ptr = (void*)host_read_adc,
        .signature = "()i",
        .attachment = NULL
    },
    {
        .symbol = "set_buzzer",
        .func_ptr = (void*)host_set_buzzer,
        .signature = "(i)",
        .attachment = NULL
    }
};
